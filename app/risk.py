...
_reflatten_at: dict[tuple[int, str], float] = {}
...
async def flatten_account(session: Any, account: dict[str, Any]) -> tuple[int, int, list[str]]:
    """Cancel working orders, liquidate once per symbol, then confirm broker-flat state."""
...
async def check_area(area_id: int, sessions: list[Any], snapshots: list[dict[str, Any]], *,
    ...
    by_aid: dict[int, list[tuple[Any, dict[str, Any]]]] = {}
    ...
    for sess in sessions:
        for a in sess.accounts:
            if a.get("id"):
                ...
                state.log_event("error", f"Risk guard: {a.get('spec')} has rules but no broker account id — it is NOT guarded; run Connect & Verify on its login")
