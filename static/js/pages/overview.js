...
export default {
  ...
  render(root, { navigate }) {
    ...
    const k = {
      trading: kpi(t("Trading"), "zap"),
      logins: kpi(t("Logins"), "key"),
      accounts: kpi(t("Trade accounts"), "users"),
      trades: kpi(t("Active trades"), "activity"),
      discord: can(me, "discord") ? kpi(t("Discord listener"), "discord") : null,
      stream: kpi(t("Live updates"), "wifi"),
    };
    ...
    const sessions = dataTable({
      ...
      columns: [
        ...
        { label: t("Last error"), render: (x) => h("span", { class: x.last_error ? "neg" : "muted" }, x.last_error ? errText(x.last_error) : "—") },
      ],
    });
    ...
    function paintTicketAccounts(s) {
      ...
      if (key !== ticketOptionsKey) {
        ticketOptionsKey = key;
        const cur = acctSel.value;
        clear(acctSel);
        for (const o of options) acctSel.append(h("option", { value: o.value, disabled: o.disabled }, o.label));
      ...
      if (!list.length) { ticketHint.textContent = t("Connect a login under Settings → Broker Accounts."); ticketHint.className = "muted"; return; }
    ...
    function paintRollover(list) {
      ...
      rollover.append(
        ...
        h("ul", { style: "margin:6px 0 8px 18px" }, items.map((w) => h("li", null,
          h("code", null, w.tv_symbol), " → ", h("code", null, w.contract), ` (${DATE_KIND[w.date_kind] || w.date_kind} ${w.date}, `,
          w.days_left < 0 ? t("{n}d ago", { n: -w.days_left }) : w.days_left === 0 ? t("today") : t("in {n}d", { n: w.days_left }),
    ...
    async function pinThreshold(a) {
      ...
      const v = await promptDialog({ title: t("Pin the drawdown threshold · {spec}", { spec: maskAccount(a.spec) }),
        body: t("The trailing threshold as shown by your prop firm / broker (e.g. 48100). The bridge trails it forward from there. Leave empty to reset the tracker."),
        label: t("Threshold"), value: cur, placeholder: "48100", confirmText: t("Pin"), type: "text" });
      if (v === null) return;
    ...
    const ddCell = (a) => {
      ...
      return h("span", { title: tip },
        ...
        h("span", { class: "sub" }, a.dd_level != null ? [t("level "), fmtMoney(a.dd_level, 0), " "] : null, mode, " ", pin,
    ...
    function paintHead() {
      ...
      for (const [key, label] of PNL_COLS) {
        ...
        pnlHead.append(h("button", { type: "button", class: `pnl-sort${active ? " active" : ""}`, title: t("Sort by {column}", { column: label.toLowerCase() }), onClick: () => {
    ...
    const riskTag = (a) => a.risk && a.risk.locked ? tag(t("locked"), "warn") : null;
    ...
    function paintPnl(p, keepPrev = false) {
      ...
      pnlSub.append(t("Today · realised "), money(p.realized), t(" · open "), money(p.open), t(" · week "), money(p.week),
        t(" · {n} active", { n: accounts.length - idle }), idle ? t(" · {n} idle", { n: idle }) : "");
      if (p.error) pnlSub.append(h("span", { class: "neg" }, " · ", errText(p.error)));
      paintHead();
      clear(pnlRows);
      ...
      for (const a of sortAccounts(accounts)) {
        ...
        pnlRows.append(h("div", { class: `pnl-row${isIdle(a) ? " idle" : ""}` },
          ...
          cell(t("balance"), h("span", null, fmtMoney(a.cash, 2)), ch("cash")),
          cell(t("max drawdown"), ddCell(a), ch("dd_room"))));
      }
    ...
    const unsubs = [
      ...
      store.subscribe("status", (s) => {
        ...
        k.trades.set(String(rows.length), rows.length ? "on" : "", rows.length ? t("managed by the bridge") : t("flat"));
        active.update(rows);
      ...
      store.subscribe("positions", (p) => {
        if (p && p.error) { positions.update([], { force: true }); const td = positions.tbody.firstChild.firstChild; clear(td); td.append(errText(p.error)); return; }
        positions.update(p || []);
      }, { immediate: true }),
      ...
      k.discord ? store.subscribe("discordStatus", (d) => {
        ...
        const label = d.state === "token_invalid" ? t("Token rejected") : d.health === "down" ? t("Offline")
          : ({ connected: t("Connected"), connecting: t("Connecting…"), disabled: t("Disabled"), error: t("Error"), library_missing: t("No library"), stopped: t("Stopped"), not_entitled: t("Not enabled") }[d.state] || d.state);
        k.discord.set(d.enabled ? label : t("Off"), d.state === "connected" ? "on" : (d.health === "down" || d.state === "error") ? "off" : "", d.user ? t("as {user}", { user: d.user }) : t("{n} channel(s) watched", { n: (d.watched_channels || []).length }));
      }, { immediate: true }) : null,
    ].filter(Boolean);
