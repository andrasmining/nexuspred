...
const fmtTime = (iso) => { const d = new Date(iso); return Number.isNaN(d.getTime()) ? "—" : d.toLocaleString(locale(), { weekday: "short", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }); };
...
export default {
  title: t("News & Calendar"),
  gate: "admin",
  render(root) {
    let settings = null;
    const enabled = h("input", { type: "checkbox", class: "switch" });
    const currencies = h("input", { class: "input-sm", placeholder: t("USD, EUR"), style: "width:180px" });
    ...
    const action = h("select", { class: "input-sm" }, h("option", { value: "block" }, t("Block new entries")), h("option", { value: "flatten" }, t("Block entries and flatten open positions at window start")));
    ...
    function renderManual() {
      ...
      manual.forEach((m, i) => manualBox.append(h("div", { style: "display:flex;gap:8px;align-items:center;flex-wrap:wrap" },
        h("input", { class: "input-sm", placeholder: t("Title (e.g. Powell speech)"), value: m.title, style: "min-width:220px", onInput: (e) => { m.title = e.target.value; } }),
        ...
        h("button", { type: "button", class: "btn btn-ghost btn-sm", title: t("Remove"), onClick: () => { manual.splice(i, 1); renderManual(); } }, icon("trash")))));
      manualBox.append(h("button", { type: "button", class: "btn btn-sm", onClick: () => { manual.push({ title: "", at: "" }); renderManual(); } }, icon("plus"), t("Add manual event")));
    }
    function fill(s) {
      settings = s;
    ...
    function paintStatus(st) {
      ...
      if (!st.enabled) statusBox.append(t("News lock is off. Entries are never blocked by the calendar."));
    ...
    root.append(
      pageHead(t("News & Calendar"), t("No new entries around high-impact releases (FOMC, CPI, NFP …). Closes, stop moves and the copy mirror always run — a position is never left unmanaged. The calendar comes from ForexFactory's weekly feed and is refreshed every six hours; the list of events is on the Calendar page (Monitoring).")),
